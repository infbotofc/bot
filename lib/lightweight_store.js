...
if (process.env.FIREBASE_URL) {
  try {
    ...
    const { ref, set, get, child, remove } = require('firebase/database')
    ...
    adapters.firebase = {
      async save(jid, id, msg) {
        try {
          await set(ref(db, `messages/${id}`), {
            jid,
            ts: Date.now(),
            data: compress(msg).toString('base64')
          })
        } catch (e) {
      ...
      async load(jid, id) {
        try {
          const snapshot = await get(child(ref(db), `messages/${id}`))
          if (snapshot.exists()) {
            const data = snapshot.val()
            return decompress(Buffer.from(data.data, 'base64'))
          }
          return null
        } catch (e) {
      ...
      async incrementCount(chatId, userId) {
        try {
          const countRef = ref(db, `counts/${chatId}/${userId}`)
          const snapshot = await get(countRef)
          const currentCount = snapshot.exists() ? snapshot.val() : 0
          await set(countRef, currentCount + 1)
        } catch (e) {
          console.error('[FIREBASE] Increment error:', e.message)
        }
      },
      async getCount(chatId, userId) {
        try {
          const snapshot = await get(ref(db, `counts/${chatId}/${userId}`))
      ...
      async getAllCounts() {
        try {
          ...
          const result = { isPublic: true, messageCount: snapshot.exists() ? snapshot.val() : {} }
          const meta = await get(ref(db, 'metadata/isPublic'))
          if (meta.exists()) result.isPublic = meta.val() === 'true'
          return result
        } catch (e) {
      ...
      async setPublicMode(isPublic) {
        try {
          await set(ref(db, 'metadata/isPublic'), isPublic.toString())
        } catch (e) {}
      },
      async setMetadata(key, value) {
        try {
          await set(ref(db, `metadata/${key}`), value.toString())
        } catch (e) {}
      },
      async getMetadata(key) {
        try {
          const snapshot = await get(ref(db, `metadata/${key}`))
          return snapshot.exists() ? snapshot.val() : null
        } catch (e) { return null }
      },
      async saveContact(jid, contact) {
        try {
          await set(ref(db, `contacts/${jid.replace(/\./g, '_')}`), { ...contact, ts: Date.now() })
        } catch (e) {}
      },
      async getContact(jid) {
        try {
          const snapshot = await get(ref(db, `contacts/${jid.replace(/\./g, '_')}`))
          return snapshot.exists() ? snapshot.val() : null
        } catch (e) { return null }
      },
      ...
      async saveChat(jid, chat) {
        try {
          await set(ref(db, `chats/${jid.replace(/\./g, '_')}`), { ...chat, ts: Date.now() })
        } catch (e) {}
      },
      async getChat(jid) {
        try {
          const snapshot = await get(ref(db, `chats/${jid.replace(/\./g, '_')}`))
          return snapshot.exists() ? snapshot.val() : null
        } catch (e) { return null }
      },
      async getAllChats() {
      ...
      async deleteChat(jid) {
        try {
          await remove(ref(db, `chats/${jid.replace(/\./g, '_')}`))
        } catch (e) {}
      },
      async saveSetting(chatId, key, value) {
        try {
          await set(ref(db, `settings/${chatId.replace(/\./g, '_')}/${key}`), value)
        } catch (e) {}
      },
      async getSetting(chatId, key) {
        try {
          const snapshot = await get(ref(db, `settings/${chatId.replace(/\./g, '_')}/${key}`))
          return snapshot.exists() ? snapshot.val() : null
        } catch (e) { return null }
      },
      async getAllSettings(chatId) {
        try {
          const snapshot = await get(ref(db, `settings/${chatId.replace(/\./g, '_')}`))
          return snapshot.exists() ? snapshot.val() : {}
        } catch (e) { return {} }
      },
      async cleanup() {
    ...
    printLog('store', '🔥 Firebase Database enabled - All data syncing to Cloud')
  } catch (err) {
    printLog('warning', `Firebase initialization failed: ${err.message}`)
  }
}
    ...
    adapters.postgres = {
      ...
      async resetAll() {
        try {
          ...
          const client = await pool.connect()
          try {
            await client.query('TRUNCATE TABLE messages, message_counts, metadata, contacts, chats, settings CASCADE')
            console.log('[POSTGRES] All data reset successfully')
          } finally {
            client.release()
          }
        } catch (e) {
          console.error('[POSTGRES] Reset error:', e.message)
          throw e
        }
      }
    }
  ...
  async getSetting(chatId, key) {
    if (backend === 'memory') {
      ...
      try {
        if (fs.existsSync(filePath)) {
          const data = JSON.parse(fs.readFileSync(filePath, 'utf-8'))
          return Object.prototype.hasOwnProperty.call(data, chatId) ? data[chatId] : null
        }
        return null
      } catch (e) {
      ...
      try {
        if (fs.existsSync(dataDir)) {
          ...
          for (const file of files) {
            ...
            if (Object.prototype.hasOwnProperty.call(data, chatId)) {
